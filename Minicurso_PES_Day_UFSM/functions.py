...
def define_3ph_pvsystem_with_transformer(dss, bus, kv, kva, pmpp):
    """
    Add a PVSystem through a step-up transformer
    ...
    dss.text("New line.PV_{} phases=3 bus1={} bus2=PV_sec_{} switch=yes".format(
        bus, bus, bus))
    ...
    dss.text(
        "New monitor.PV_{}_V element=transformer.PV_{} terminal=2 mode=0".format(bus, bus))
    dss.text(
        "New monitor.PV_{}_P element=PVSystem.PV_{} terminal=1 mode=1 ppolar=no".format(bus, bus))
    dss.text(
        "New monitor.PV_{}_M element=PVSystem.PV_{} terminal=1 mode=3".format(bus, bus))
